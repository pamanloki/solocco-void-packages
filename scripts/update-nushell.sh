...
TEMPLATE="srcpkgs/nushell/template"
...
# nushell tags are plain version numbers, no 'v' prefix
LATEST_VERSION="${LATEST_TAG#v}"

if [ "$LATEST_VERSION" = "$CURRENT_VERSION" ]; then
  echo "nushell is up to date (${CURRENT_VERSION})"
  exit 0
fi

echo "Updating nushell: ${CURRENT_VERSION} -> ${LATEST_VERSION}"
