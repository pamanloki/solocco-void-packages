...
LATEST_VERSION=$(gh api "repos/$REPO/releases?per_page=50" \
    --jq '[.[] | select(.prerelease == false) | select(any(.assets[]; .name | test("^brave-origin-[0-9].*x86_64\\.rpm$")))] | first | .tag_name' \
    | sed 's/^v//')
...
URL=$(gh api "repos/$REPO/releases?per_page=50" \
    --jq "[.[] | select(.tag_name == \"v${LATEST_VERSION}\")] | first | .assets[] | select(.name | test(\"^brave-origin-[0-9].*x86_64\\\\.rpm$\")) | .browser_download_url")
if [ -z "$URL" ]; then
