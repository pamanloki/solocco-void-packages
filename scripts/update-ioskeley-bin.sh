...
wget -q "$BASE/IoskeleyMonoTerm-NerdFont.tar.xz" -O /tmp/term.tar.xz

CS1=$(sha256sum /tmp/ttf.tar.xz  | cut -d' ' -f1)
CS2=$(sha256sum /tmp/nerd.tar.xz | cut -d' ' -f1)
CS3=$(sha256sum /tmp/term.tar.xz | cut -d' ' -f1)

rm /tmp/ttf.tar.xz /tmp/nerd.tar.xz /tmp/term.tar.xz
